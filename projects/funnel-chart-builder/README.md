...
- 手机可横向滚动图表，保持导出布局完整。
- 本应用无模型调用，不需要 Gemini API Key。使用 yeuhub_funnel_history_v1 保存历史快照；刷新读取最近保留版本，未保留修改不会自动保存。

此前聊天页面相关源码仍在 `agent/` 中保留，当前入口不加载。

本次布局：左侧环节名单行展示，右侧待办独立紧凑排列、全文展开，以曲线对应漏斗环节。空待办不占位。

默认模板更新为六层客户漏斗（108、101、94、61、50、40）。数值在图形内编辑，占比统一为当前层/首层；首层为0时显示横杠。valueNote、percentNote 为手动备注，和历史记录一起保存；旧记录缺少备注时兼容为空字符串。已有历史优先恢复，可用“使用默认六层”加载新模板，不清空历史。
