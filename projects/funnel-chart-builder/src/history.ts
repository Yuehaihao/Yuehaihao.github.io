...
export function readHistory(storage: Pick<Storage, 'getItem'>): Snapshot[] {
  ...
  if (!Array.isArray(data) || !data.every(s => s && typeof s.id === 'string' && typeof s.title === 'string' && typeof s.savedAt === 'string' && Number.isFinite(Date.parse(s.savedAt)) && Array.isArray(s.levels) && s.levels.length >= 1 && s.levels.length <= 20 && new Set(s.levels.map((l: FunnelLevel) => l.id)).size === s.levels.length && s.levels.every((l: FunnelLevel) => l && typeof l.id === 'string' && typeof l.label === 'string' && Number.isFinite(l.value) && l.value >= 0 && /^#[0-9a-f]{6}$/i.test(l.color) && typeof l.actionItem === 'string' && typeof l.hasActionItem === 'boolean'))) throw new Error('历史记录无法读取，原数据未修改。');
  return data;
}
